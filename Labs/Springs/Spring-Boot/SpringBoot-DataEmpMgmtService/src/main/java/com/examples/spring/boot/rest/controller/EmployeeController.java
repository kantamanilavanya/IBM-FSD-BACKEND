...
import com.examples.spring.boot.rest.exception.LoginException;
import com.examples.spring.boot.rest.exception.ValidationException;
import com.examples.spring.boot.rest.model.Employee;
import com.examples.spring.boot.rest.model.ErrorDetails;
...
//		}
	  ...
	  empService.add(employee);
		...
		URI locationUri = new URI("/SpringRESTEmployeeCRUDEx/employees/"+employee.getId());
...
//		return ResponseEntity.ok().cacheControl(CacheControl.maxAge(10, TimeUnit.MINUTES).cachePrivate()).body(empService.list());
	  ...
	  @GetMapping("/{id}") public HttpEntity<Employee> getEmployee(@PathVariable
	...
	@CrossOrigin(origins = "*")
	@PutMapping("/{id}")
	public HttpEntity<? extends Object> updateEmployee(@RequestBody Employee employee, @PathVariable int id,
